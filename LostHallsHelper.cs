...
internal class LostHallsHelper {
		...
		while (true) {
			...
			switch ((num3 = (uint) num ^ 0x84FFC1E6u) % 14u) {
				...
				default:
					return;
				case 0u:
					ChangeSizeStat(gameObject._Status);
				...
				case 6u: {
					int num7;
					int num8;
					if (gameObject._ObjectType >= _RgjGsT3X2SuduoqBsF2FKpFbcnD) {
						num7 = -1108408877;
						num8 = num7;
					}
					else {
				...
				case 5u: {
					int num5;
					int num6;
					if (gameObject._ObjectType <= _f3ybM4yNmpSNGUhiusVkVdI7lin) {
						num5 = 1594249089;
						num6 = num5;
					}
					else {
	...
	private void ChangeSizeStat(Status status)
	{
				...
				while (true) {
					...
					switch ((num3 = (uint) num ^ 0x1B1E3BD8u) % 7u) {
						...
						case 5u: {
							int num4;
							int num5;
							if (StatType._agQ5CSGcaJRx3TN8BU6OBzWi5yT(current._StatType,
								StatType._zMlSnmW3LLSxr4x3sZAbRbWfh8Y)) {
								num4 = -1730027036;
								num5 = num4;
							}
							else {
